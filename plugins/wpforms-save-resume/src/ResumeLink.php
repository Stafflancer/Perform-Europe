...
class ResumeLink {
	...
	public function resume_link( $message, $form_data, $fields = [], $entry_id = '' ) {
		...
		$link = sprintf(
			'<div style="text-align:center"><a href="%1$s" class="inline" style="text-decoration:none;padding:7px 15px;border-radius:3px;background-color:%2$s;border:1px solid %2$s;color:%3$s;">%4$s</a></div>',
			esc_url( $hash_url ),
