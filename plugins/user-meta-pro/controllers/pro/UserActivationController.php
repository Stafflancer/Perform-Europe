...
class UserActivationController
{
    ...
    function __construct()
    {
        ...
        /**
         * User row actions for multisite
         */
        ...
        /**
         * This hook can be found on:
         * user.php > wp_signon(), wp_authenticate(), wp_authenticate_username_password() > filter:authenticate
         * wp_includes/default-filters.php > wp_authenticate_username_password
         */
    ...
    function loadUsersPage()
    {
        ...
        $user = ! empty($_REQUEST['user']) ? sanitize_text_field($_REQUEST['user']) : null;
        $users = ! empty($_REQUEST['users']) ? sanitizeDeep($_REQUEST['users']) : null;
        $action = ! empty($_REQUEST['action']) ? sanitizeDeep($_REQUEST['action']) : null;
    ...
    function userAdminCustomization()
    {
        ...
        /**
         * different admin url based on site type (multisite or not)
         */
        $activeUrl = is_multisite() ? network_admin_url('users.php?um_status=active') : admin_url('users.php?um_status=active');
        $inactiveUrl = is_multisite() ? network_admin_url('users.php?um_status=inactive') : admin_url('users.php?um_status=inactive');
        $pendingUrl = is_multisite() ? network_admin_url('users.php?um_status=pending') : admin_url('users.php?um_status=pending');
        $pendingApprovalUrl = is_multisite() ? network_admin_url('users.php?um_status=pending_approval') : admin_url('users.php?um_status=pending_approval');
        $pendingVerificationUrl = is_multisite() ? network_admin_url('users.php?um_status=pending_email_verification') : admin_url('users.php?um_status=pending_email_verification');
        ...
        $html = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
        $html .= "<li><a href=\"" . $activeUrl . "\">{$labels['active']} ({$countActive})</a></li>";
        $html .= "<li> | <a href=\"" . $inactiveUrl . "\">{$labels['inactive']} ({$countInactive})</a></li>";

        if ($countPending)
            $html .= "<li> | <a href=\"" . $pendingUrl . "\">{$labels['pending']} ({$countPending})</a></li>";
    ...
    AND  (mt1.meta_key = 'user_meta_email_verification_code' AND mt1.meta_value = '') )");
                ...
                if ($countApproval)
                    $html .= "<li> | <a href=\"" . $pendingApprovalUrl . "\">{$labels['pending_approval']} ({$countApproval})</a></li>";

                if ($countVerification)
                    $html .= "<li> | <a href=\"" . $pendingVerificationUrl . "\">{$labels['pending_verification']} ({$countVerification})</a></li>";
            }
        }
            ...
            if (in_array($action, array(
                ...
                if ($userID) {
                    ...
                    if ($_GET['success']) {
                        ...
                        if ((! empty($_GET['count']) ? $_GET['count'] : null) > 1)
                            $status = sprintf(__('%1$s users have been %2$s', $userMeta->name), sanitize_text_field($_GET['count']), $status);
                        else
