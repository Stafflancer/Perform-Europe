...
class BackendProfileController
{
    ...
    function profileField($user)
    {
        ...
        elseif ($pagenow == 'user-edit.php')
            $userID = ! empty($_REQUEST['user_id']) ? esc_attr($_REQUEST['user_id']) : null;
        ...
        $backendFields = ! empty($settings['backend_profile']['fields']) ? $settings['backend_profile']['fields'] : null;
        ...
        foreach ($backendFields as $fieldID) {
            ...
            if (($i == 1) || ((! empty($fields[$fieldID]['field_type']) ? $fields[$fieldID]['field_type'] : null) != 'section_heading')) {
            ...
            if ($fields[$fieldID]['field_type'] == 'section_heading') {
                ...
                echo "<h2>" . $fields[$fieldID]['field_title'] . "</h2> <table class='form-table'><tbody>";
                $inTable = true;
                continue;
            }

            $fieldName = ! empty($fields[$fieldID]['meta_key']) ? $fields[$fieldID]['meta_key'] : null;
            if (! $fieldName)
                $fieldName = $fields[$fieldID]['field_type'];

            $fields[$fieldID]['id'] = $fieldID;
            $fields[$fieldID]['field_name'] = $fieldName;
            $fields[$fieldID]['field_value'] = ! empty($user->$fieldName) ? $user->$fieldName : null;
...
<?php
    ...
    function _hideBackendFields()
    {
        ...
        $hide_fields = ! empty($backend_profile['hide_fields']) ? $backend_profile['hide_fields'] : null;
