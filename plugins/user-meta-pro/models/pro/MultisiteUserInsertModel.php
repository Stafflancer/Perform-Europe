...
class MultisiteUserInsertModel
{
    ...
    public function validateBlogSignup()
    {
        ...
        if (is_multisite() && wp_verify_nonce((! empty($_POST['um_newblog']) ? $_POST['um_newblog'] : null), 'blogname') && ! empty($_POST['blogname'])) {
            $blogData = wpmu_validate_blog_signup(sanitize_text_field($_POST['blogname']), sanitize_text_field($_POST['blog_title']));
    ...
    public function addUserToBlog(array $userData)
    {
        ...
        if (is_multisite()) {
            ...
            if (! empty($registrationSettings['add_user_to_blog'])) {
                $userID = userIDfromUsernameOrEmail($userData);
                if ($userID) {
