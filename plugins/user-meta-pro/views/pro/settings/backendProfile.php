...
$html = "<br />";
$html .= "<h6>" . __('Hide WordPress default fields', $userMeta->name) . "</h6>";
...
foreach ($wpBackendFields as $fieldsGroup) {
    ...
    foreach ($fieldsGroup as $fieldKey => $fieldValue) {
        ...
        $input .= Html::checkbox(! empty($backend_profile['hide_fields'][$fieldKey]), [
            'name' => "{$fieldPrefix}[hide_fields][{$fieldKey}]",
            'id' => "{$fieldPrefix}_hide_fields_{$fieldKey}",
            '_after' => '&nbsp' . $fieldValue,
            '_enclose' => [
                'label',
...
if (is_array($fields)) {
    foreach ($fields as $key => $field) {
        if (empty($field['meta_key'])) {
            if (! in_array((! empty($field['field_type']) ? $field['field_type'] : null), array(
                'user_registered',
                'section_heading',
                'user_avatar'
            )))
...
$html .= '<h6>' . __('Extra fields in backend profile', $userMeta->name) . '</h6>';
